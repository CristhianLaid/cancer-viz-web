...
export const SurvivalAnalysis: React.FC<SurvivalAnalysisProps> = ({ data=[] }) => {
    const [activeTab, setActiveTab] = useState<'chart' | 'settings' | 'stats'>('chart');
    ...
    const applySurvivalType = (survivalCurve: { x: number; y: number }[]) => {
        switch (survivalType) {
            case "optimistic":
                return survivalCurve.map((point) => ({ x: point.x, y: Math.pow(point.y, 0.7) }));
            case "pessimistic":
                return survivalCurve.map((point) => ({ x: point.x, y: Math.pow(point.y, 1.3) }));
            case "earlyDrop":
                return survivalCurve.map((point) => ({ x: point.x, y: point.y * Math.exp(-point.x / 50) }));
            case "lateDrop":
                return survivalCurve.map((point) => ({ x: point.x, y: point.y * (1 - Math.exp(-point.x / 50)) }));
            default:
            ...
            <CardContent>
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            {selectedCancerTypes.map((cancerType) => (
                                                <th key={cancerType} className="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                                    {cancerType}
                                                </th>
                                            ))}
                                        </tr>
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {[0, 12, 24, 36, 48, 60].map((month) => (
                                            <tr key={month}>
                                                ...
                                                {selectedCancerTypes.map((cancerType) => (
                                                    <td key={cancerType} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                        {survivalData.find((d) => d.label === cancerType)?.data.find((p) => p.x >= month)?.y.toFixed(2) || "N/A"}
                                                    </td>
                                                ))}
                                            </tr>
                                        ))}
                    ...
                    <TabsContent value="settings" className="space-y-4">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold mb-2">Tipo de Supervivencia:</h3>
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center space-x-2">
                                    ...
                                    />
                                    <label htmlFor="confidenceInterval">Intervalo de confianza</label>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <input
                                        type="checkbox"
                    ...
                    <TabsContent value="stats" className="space-y-4">
                        <div>
                            <h3 className="text-lg font-semibold mb-2">Estadísticas de Supervivencia</h3>
                            {/* Placeholder for statistical data */}
                            <p className="text-gray-700">Aquí se mostrarán estadísticas sobre los tipos de cáncer seleccionados.</p>
                        </div>
                    </TabsContent>
                </Tabs>
            </CardContent>
